...
pdf("overnight_dsd_stimulus_efficiency.pdf", w=12,h=6)
...
ggetho(int_dt, aes(y=max_velo,color=sex), summary_FUN=median) + 
			stat_pop_etho() + 
			facet_grid( interval ~ sex) +
			scale_y_continuous(name="Max velocity post stim") + scale_y_sqrt() 
...
# interaction_id is very good predictor of failure!!
#ggplot( rejoin(int_dt), aes(y=as.numeric(response), x= interaction_id)) + geom_point(alpha=.01) + geom_smooth() + facet_grid( interval ~ sex)

dev.off()

pdf("/tmp/overnight_dsd_stimulus_efficiency_indiv.pdf", w=8,h=8)
rejoin(int_dt)[sdi==10][, 
				print(ggplot( .SD, aes(y=as.numeric(response), x= interaction_id)) + 
											geom_point(alpha=.01) + 
											geom_smooth() + 
											scale_x_continuous(limits=c(0, hours(12)/20)))
											, by=id]
