...
for (d in dates){
  ...
  pl <- ggetho(dt, aes(y=paste(sex, id), z=asleep)) + 
        stat_tile_etho() + 
        ggtitle(d)
...
layers <- function(palette = CONTROL_SD_PALETTE, annotate=TRUE){
  ...
  if(annotate)
    out <- c(out, list(
        annotate("segment",y = .9, yend = .9,   x = 0, xend = hours(8),   colour = "black",alpha=0.5,size=3),
        annotate("text",y=0.95,x=hours(4), label="treatment")
    ))
    out
}
...
pdf("dsd_zt0_8_rebound.pdf", w=12,h=6)
for(p_name in names(all_pl_objs)){
    ...
    htmlwidgets::saveWidget(plotly::as_widget(p),
                sprintf("./dsd_zt0_8_rebound-%s.html", p_name))
}
dev.off()
